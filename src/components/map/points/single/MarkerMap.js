...
export default function MarkerMap(props) {
  const { onChildClick } = useContext(store)

  if (props.motive === null) {
    return (
      <div
        className={clx('sale-marker', {
          'sale-marker--hover': props.hover || props.$hover,
        })}
        onClick={(event) => {
    ...
    return (
      <div
        className={clx('no-sale-marker', {
          'no-sale-marker--hover': props.hover || props.$hover,
        })}
        onClick={(event) => {
          onChildClick(event, props)
          props.mapInstance.setZoom(15)
...
export function MarkerInfoSale({ client_name, client_db_ref, direction }) {
  return (
    ...
    >
      <motion.div className="place-marker-redirect-link">
        {' '}
        {client_name}
      </motion.div>
      <motion.div className="sale-marker-title"> {direction}</motion.div>
    </motion.div>
  )
}
